 ...
 * └── {did}/
 *     ├── display_name
 *     ├── manifesto / manifesto_cid
 *     ├── preferences / preferences_cid
 *     ├── decisions / decisions_cid
 *     └── knowledge / knowledge_cid
 */

/** Root Ensue namespace — set to empty string (no prefix). */
export const ENSUE_PREFIX = '';

/** Key helper — pass-through (no prefix applied). */
export const K = (key: string) => key;

export const MEMORY_KEYS = {
  // Coordinator (ephemeral, current round)
  COORDINATOR_TALLY: K('coordination/coordinator/tally'),
  COORDINATOR_STATUS: K('coordination/coordinator/status'),
  COORDINATOR_PROPOSAL_ID: K('coordination/coordinator/proposal_id'),

  // Config
  CONFIG_TASK_DEFINITION: K('coordination/config/task_definition'),
  CONFIG_CONTRACT_ADDRESS: K('coordination/config/contract_address'),
} as const;
...
export function getWorkerKeys(workerId: string) {
  return {
    STATUS: K(`coordination/tasks/${workerId}/status`),
    RESULT: K(`coordination/tasks/${workerId}/result`),
    TIMESTAMP: K(`coordination/tasks/${workerId}/timestamp`),
    ERROR: K(`coordination/tasks/${workerId}/error`),
    VERIFICATION_PROOF: K(`coordination/tasks/${workerId}/verification_proof`),
  };
}
...
export function getAllWorkerStatusKeys(workerIds: string[]): string[] {
  return workerIds.map(id => K(`coordination/tasks/${id}/status`));
}
...
export function getAllWorkerResultKeys(workerIds: string[]): string[] {
  return workerIds.map(id => K(`coordination/tasks/${id}/result`));
}
...
export function getProposalKeys(proposalId: string) {
  const base = K(`coordination/proposals/${proposalId}`);
  return {
    CONFIG: `${base}/config`,
    STATUS: `${base}/status`,
    TALLY: `${base}/tally`,
...
export function getProposalWorkerKeys(proposalId: string, workerId: string) {
  const base = K(`coordination/proposals/${proposalId}/workers/${workerId}`);
  return {
    RESULT: `${base}/result`,
 ...
 */
export const PROPOSAL_INDEX_KEY = K('coordination/proposals/_index');
...
export const getCoordinatorSnapshotKey = (proposalId: number | string): string =>
  K(`coordination/coordinator/worker_snapshot_${proposalId}`);
...
export function getAgentRegistryKeys(agentId: string) {
  const base = K(`coordination/agents/${agentId}`);
  return {
