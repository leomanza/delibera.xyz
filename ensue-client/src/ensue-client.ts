import axios, { AxiosInstance, AxiosError } from 'axios';
import { IMemoryClient, EnsueClientConfig, Logger } from './types';

const DEFAULT_BASE_URL = 'https://api.ensue-network.ai/';
const DEFAULT_TIMEOUT = 15000;

/** Default logger — delegates to console */
const defaultLogger: Logger = {
  error: (msg, ...args) => console.error(msg, ...args),
  info: (msg, ...args) => console.log(msg, ...args),
};

/**
 * Ensue Memory Network API Client (JSON-RPC 2.0 over SSE)
 *
 * All operations use POST to the configured base URL.
 * Responses are Server-Sent Events with JSON-RPC payloads.
 *
 * Implements IMemoryClient — consumers should program against that interface.
 */
export class EnsueClient implements IMemoryClient {
  private client: AxiosInstance;
  private requestId = 0;
  private logger: Logger;

  /**
   * Create an EnsueClient with explicit configuration.
   *
   * @param config - EnsueClientConfig with apiKey (required) and optional overrides.
   *                 Also accepts a plain string for backward compatibility (treated as apiKey).
   */
  constructor(config: EnsueClientConfig | string) {
    // Backward compat: accept a plain string as the API key
    const resolved: EnsueClientConfig =
      typeof config === 'string' ? { apiKey: config } : config;

    if (!resolved.apiKey) {
      throw new Error('Ensue API key is required');
    }

    this.logger = resolved.logger ?? defaultLogger;

    this.client = axios.create({
      baseURL: resolved.baseURL ?? DEFAULT_BASE_URL,
      headers: {
        'Authorization': `Bearer ${resolved.apiKey}`,
        'Content-Type': 'application/json',
      },
      timeout: resolved.timeout ?? DEFAULT_TIMEOUT,
      responseType: 'text', // SSE returns text/event-stream
    });
  }

  /**
   * Send a JSON-RPC request and parse the SSE response
   */
  private async rpc(method: string, args: Record<string, unknown>): Promise<any> {
    const id = ++this.requestId;
    const response = await this.client.post('/', {
      jsonrpc: '2.0',
      method: 'tools/call',
      params: {
        name: method,
        arguments: args,
      },
      id,
    });

    // Parse SSE response: "data: {json}\n\n"
    const raw = typeof response.data === 'string' ? response.data : JSON.stringify(response.data);
    const lines = raw.split('\n').filter((l: string) => l.startsWith('data: '));
    if (lines.length === 0) return null;

    // Take the last data line (final result)
    const jsonStr = lines[lines.length - 1].replace(/^data: /, '');
    const parsed = JSON.parse(jsonStr);

    // Check for JSON-RPC error (top-level error field)
    if (parsed?.error) {
      throw new Error(parsed.error.message || 'Ensue API error');
    }

    // Check for application-level error
    if (parsed?.result?.isError) {
      throw new Error(parsed.result.content?.[0]?.text || 'Ensue API error');
    }

    return parsed;
  }

  /**
   * Extract structured content or text content from response
   */
  private extractData(response: any): any {
    if (!response?.result) return null;
    // Prefer structuredContent for typed data
    if (response.result.structuredContent !== undefined) {
      return response.result.structuredContent;
    }
    // Fallback to text content
    const textContent = response.result.content?.find((c: any) => c.type === 'text');
    if (textContent?.text) {
      try {
        return JSON.parse(textContent.text);
      } catch {
        return textContent.text;
      }
    }
    return null;
  }

  /**
   * Create a new memory entry
   */
  async createMemory(key: string, value: string | object, description?: string): Promise<void> {
    const valueStr = typeof value === 'string' ? value : JSON.stringify(value);
    try {
      await this.rpc('create_memory', {
        items: [{
          key_name: key,
          value: valueStr,
          description: description || `Memory at ${key}`,
        }],
      });
    } catch (error) {
      this.handleError('createMemory', key, error);
    }
  }

  /**
   * Read a single memory value by key
   */
  async readMemory(key: string): Promise<string | null> {
    try {
      const response = await this.rpc('get_memory', { key_names: [key] });
      const data = this.extractData(response);
      if (!data) return null;
      // Response: { results: [{ key_name, value, ... }] } or array
      if (Array.isArray(data)) {
        return data[0]?.value || null;
      }
      if (data.results && Array.isArray(data.results)) {
        return data.results[0]?.value || null;
      }
      if (data.value !== undefined) return data.value;
      return null;
    } catch (error) {
      if (this.isNotFoundError(error)) return null;
      this.handleError('readMemory', key, error);
      return null;
    }
  }

  /**
   * Read multiple memory values by keys
   */
  async readMultiple(keys: string[]): Promise<Record<string, string>> {
    const result: Record<string, string> = {};
    try {
      const response = await this.rpc('get_memory', { key_names: keys });
      const data = this.extractData(response);
      if (!data) return result;

      const items = Array.isArray(data) ? data : (data.results || []);
      for (const item of items) {
        if (item?.key_name && item?.value !== undefined) {
          result[item.key_name] = item.value;
        }
      }
    } catch (error) {
      this.handleError('readMultiple', keys.join(', '), error);
    }
    return result;
  }

  /**
   * Update an existing memory value
   */
  async updateMemory(key: string, value: string | object): Promise<void> {
    const valueStr = typeof value === 'string' ? value : JSON.stringify(value);
    try {
      await this.rpc('update_memory', {
        key_name: key,
        value: valueStr,
      });
    } catch (error) {
      // If update fails (memory doesn't exist), try creating it
      const errMsg = error instanceof Error ? error.message : '';
      if (this.isNotFoundError(error) || errMsg.includes('not found') || errMsg.includes('does not exist')) {
        await this.createMemory(key, valueStr);
      } else {
        this.handleError('updateMemory', key, error);
      }
    }
  }

  /**
   * Delete memory entries
   */
  async deleteMemory(key: string): Promise<void> {
    try {
      await this.rpc('delete_memory', { key_names: [key] });
    } catch (error) {
      // Ignore not-found errors on delete
      if (!this.isNotFoundError(error)) {
        this.handleError('deleteMemory', key, error);
      }
    }
  }

  /**
   * List memory keys with optional prefix filter
   */
  async listKeys(prefix?: string, limit: number = 100): Promise<string[]> {
    try {
      const args: Record<string, unknown> = { limit };
      if (prefix) args.prefix = `${prefix}%`;
      const response = await this.rpc('list_keys', args);
      const data = this.extractData(response);
      if (!data) return [];

      const keys = data.keys || data;
      if (Array.isArray(keys)) {
        return keys.map((item: any) =>
          typeof item === 'string' ? item : item?.key_name
        ).filter(Boolean);
      }
      return [];
    } catch (error) {
      this.handleError('listKeys', prefix || '', error);
      return [];
    }
  }

  /**
   * Search memories by query (semantic search)
   */
  async searchMemories(query: string, limit: number = 5): Promise<any[]> {
    try {
      const response = await this.rpc('search_memories', { query, limit });
      const data = this.extractData(response);
      return Array.isArray(data) ? data : (data?.results || []);
    } catch (error) {
      this.handleError('searchMemories', query, error);
      return [];
    }
  }

  /**
   * Clear all memories with a given prefix
   */
  async clearPrefix(prefix: string): Promise<void> {
    try {
      const keys = await this.listKeys(prefix);
      if (keys.length > 0) {
        await this.rpc('delete_memory', { key_names: keys });
        this.logger.info?.(`Cleared ${keys.length} memories with prefix: ${prefix}`);
      }
    } catch (error) {
      this.handleError('clearPrefix', prefix, error);
    }
  }

  private isNotFoundError(error: any): boolean {
    if (axios.isAxiosError(error)) {
      return error.response?.status === 404;
    }
    if (error instanceof Error) {
      const msg = error.message.toLowerCase();
      return msg.includes('not found') || msg.includes('does not exist');
    }
    return false;
  }

  private handleError(operation: string, key: string, error: any): void {
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;
      this.logger.error(
        `Ensue ${operation} error for key "${key}":`,
        axiosError.response?.status,
        typeof axiosError.response?.data === 'string'
          ? axiosError.response.data.substring(0, 200)
          : axiosError.message
      );
    } else if (error instanceof Error) {
      this.logger.error(`Ensue ${operation} error for key "${key}":`, error.message);
    } else {
      this.logger.error(`Ensue ${operation} error for key "${key}":`, error);
    }
  }
}

/**
 * Convenience factory — reads ENSUE_API_KEY / ENSUE_TOKEN from process.env.
 *
 * @deprecated Use `new EnsueClient({ apiKey })` for explicit config.
 *             This factory will be moved to consumer-side application code.
 *
 * Kept for backward compatibility with existing consumers that call
 * `createEnsueClient()` with no args.
 */
export function createEnsueClient(apiKey?: string): EnsueClient {
  const key = apiKey || process.env.ENSUE_API_KEY || process.env.ENSUE_TOKEN;
  if (!key) {
    throw new Error(
      'Ensue API key not provided and ENSUE_API_KEY/ENSUE_TOKEN environment variable not set'
    );
  }
  return new EnsueClient({ apiKey: key });
}
